(() => {
  const asamOptions = ['2.1', '3.1', '3.3', '3.5', '3.7'];

  const matOptions = [
    { value: 'None', label: 'No MAT need' },
    { value: 'MethadoneContinue', label: 'Methadone Continue' },
    { value: 'MethadoneInduct', label: 'Methadone Induction' },
    { value: 'SuboxoneContinue', label: 'Suboxone Continue' },
    { value: 'SuboxoneInduct', label: 'Suboxone Induction' },
    { value: 'ExtendedReleaseNaltrexone', label: 'XR Naltrexone' }
  ];

  const statusConfig = {
    Searching: { label: 'Searching', icon: '…', chipClass: 'status-badge searching', rowClass: 'st-searching' },
    PendingReview: { label: 'Pending review', icon: '⧗', chipClass: 'status-badge pending', rowClass: 'st-pending' },
    AwaitingCallback: { label: 'Awaiting call-back', icon: '⧗', chipClass: 'status-badge pending', rowClass: 'st-pending' },
    AwaitingFax: { label: 'Awaiting fax', icon: '🖨', chipClass: 'status-badge pending', rowClass: 'st-pending' },
    PacketSent: { label: 'Packet sent', icon: '✉︎', chipClass: 'status-badge pending', rowClass: 'st-pending' },
    Accepted: { label: 'Accepted', icon: '✓', chipClass: 'status-badge accepted', rowClass: 'st-accepted' },
    Denied: { label: 'Denied', icon: '⨯', chipClass: 'status-badge denied', rowClass: 'st-denied' },
    NoBeds: { label: 'No beds', icon: '∅', chipClass: 'status-badge nobeds', rowClass: 'st-nobeds' },
    Canceled: { label: 'Canceled', icon: '—', chipClass: 'status-badge canceled', rowClass: 'st-canceled' }
  };
  ...
  const patientListData = [
    ...
    },
    {
      ...
      placement: {
        type: 'searching',
        asamLevel: '3.5',
        locTag: '3.5 — Methadone',
        specialFlags: ['MAT Continue']
      },
      mat: 'Methadone Continue',
      status: 'searching',
      transport: {
      ...
      sde: {
        ...
        assignedTo: 'kdial',
        lastUpdateTs: '2025-09-22T08:40:00',
        reassessFlag: 'N'
      }
    },
    {
    ...
    },
    {
      ...
      reassess: false,
      sde: {
        placementStatus: 'PendingDocumentation',
        facilitySelectedId: null,
        asamRequested: '3.1',
        matNeeds: 'None',
        transportStatus: 'None',
  ...
  },
  {
    ...
    "distance": 0.9,
    "phone": "215-238-2150",
    "matSupports": ["SuboxoneInduct", "SuboxoneContinue"],
    "asamLevels": ["3.1", "2.1"],
    "acuteBh": true,
    "acuteMed": false,
    "takes302": false,
    ...
    const settingsContent = `
      <div class="settings-container">
        ...
        <div class="settings-footer">
          <button class="epic-btn secondary" onclick="closeEpicModal()">Cancel</button>
  ...
  function applySettings(settings) {
    ...
    if (!settings.patientList.showStatusColors) {
      const table = document.querySelector('.patient-list-table');
      if (table) {
        table.classList.add('no-status-colors');
      }
    }
  }
  ...
  function badgeSet(facility) {
    ...
    const hasMethInduct = facility.matSupports.includes('MethadoneInduct');
    const hasSuboxoneContinue = facility.matSupports.includes('SuboxoneContinue');
    const hasSuboxoneInduct = facility.matSupports.includes('SuboxoneInduct');
    ...
    badges.push(`Suboxone induct: ${hasSuboxoneInduct ? '✓' : '✗'}`);
  ...
  function openFacilityDetails(facility, evaluation) {
    ...
    const payerList = facility.payerPlans.join(', ');
    const contact = facility.contact || {};
    const verification = formatVerifiedText(facility);
    const bedLine = facility.bed?.status ? `${facility.bed.status}${facility.bedUpdatedDate ? ' — updated ' + formatTimestamp(facility.bedUpdatedDate) : ''}` : 'Not reported';
    elements.facilityDetailsBody.innerHTML = `
      <div class="detail-section">
        <h4>Contact</h4>
        <dl>
          <dt>Name</dt><dd>${contact.name || 'Intake desk'}</dd>
          <dt>Phone</dt><dd>${contact.phone || facility.phone}</dd>
          <dt>Direct address</dt><dd>${contact.direct || '—'}</dd>
          <dt>Fax</dt><dd>${contact.fax || '—'}</dd>
        </dl>
      </div>
      <div class="detail-section">
        <h4>Transfer guidance</h4>
        <p><strong>Match status:</strong> ${evaluation.status.toUpperCase()} — ${evaluation.summary}</p>
        <p>${facility.transferInstructions || 'Follow standard packet workflow.'}</p>
        <p><strong>Capacity:</strong> ${facility.capacity || 'Unknown'}</p>
        <p><strong>Bed status:</strong> ${bedLine}</p>
      </div>
      <div class="detail-section">
        <h4>Payer & verification</h4>
        <p><strong>Payers:</strong> ${payerList}</p>
        <p>${verification}</p>
        <p><strong>Notes:</strong> ${facility.notes || 'None recorded.'}</p>
      </div>
    `;
    elements.facilityDetailsModal.classList.remove('hidden');
  }

  function openFacilityModal(facility, evaluation) {
    state.facilityCandidate = { facility, evaluation };
    elements.facilityModalTitle.textContent = facility.name;
    elements.facilityModalBody.textContent = evaluation.summary;
    elements.facilityAlternatives.innerHTML = '';
    evaluation.alternatives.forEach((alt) => {
      const button = document.createElement('button');
      button.className = 'ghost-btn';
      button.textContent = `${alt.name} (Score ${alt.score})`;
      button.addEventListener('click', () => {
        const altFacility = facilities.find((item) => item.id === alt.id);
        const altEval = evaluateFacility(altFacility, state.patient);
        openFacilityModal(altFacility, altEval);
      });
      elements.facilityAlternatives.appendChild(button);
    });

    elements.facilityOverrideAction.classList.toggle('hidden', evaluation.status !== 'block');

    if (evaluation.status === 'confirm') {
      elements.facilityPrimaryAction.textContent = 'Apply selection';
      elements.facilitySecondaryAction.textContent = 'Cancel';
      elements.facilityPrimaryAction.onclick = applyCandidateSelection;
      elements.facilitySecondaryAction.onclick = closeFacilityModal;
    } else if (evaluation.status === 'warn') {
      elements.facilityPrimaryAction.textContent = 'Proceed anyway';
      elements.facilitySecondaryAction.textContent = 'Choose alternative';
      elements.facilityPrimaryAction.onclick = applyCandidateSelection;
      elements.facilitySecondaryAction.onclick = () => {
        if (evaluation.alternatives.length) {
          const alt = evaluation.alternatives[0];
          const altFacility = facilities.find((item) => item.id === alt.id);
          const altEval = evaluateFacility(altFacility, state.patient);
          openFacilityModal(altFacility, altEval);
        }
      ...
      elements.facilityPrimaryAction.textContent = 'See alternatives';
      elements.facilitySecondaryAction.textContent = 'Cancel';
      elements.facilityPrimaryAction.onclick = () => {
        if (evaluation.alternatives.length) {
          const alt = evaluation.alternatives[0];
          const altFacility = facilities.find((item) => item.id === alt.id);
          const altEval = evaluateFacility(altFacility, state.patient);
          ...
          showToast('No confirming alternatives available.');
        }
      };
      elements.facilitySecondaryAction.onclick = closeFacilityModal;
    }

    elements.facilityOverrideAction.onclick = () => {
      if (!elements.overrideReason.value.trim()) {
        showToast('Enter override reason in the Running Note before overriding.');
        elements.overrideReason.focus();
        return;
      }
      state.patient.note.override.used = true;
      state.patient.note.override.reason = elements.overrideReason.value.trim();
      state.patient.note.override.user = 'Morgan Lee';
      addAuditEntry('Override', `${state.patient.note.override.reason}`);
      elements.facilityModal.classList.add('hidden');
      applyCandidateSelection(true);
      showToast('Override logged. Finalize unlocked for this selection.');
    };

    elements.facilityModal.classList.remove('hidden');
  }

  function applyCandidateSelection(isOverride = false) {
    const candidate = state.facilityCandidate;
    if (!candidate) return;
    state.patient.note.selectedFacilityId = candidate.facility.id;
    state.patient.note.facilityStatus = candidate.evaluation.status === 'block' && isOverride ? 'block' : candidate.evaluation.status;
    state.selectedFacility = candidate.facility;
    elements.selectedFacility.value = candidate.facility.name;
    if (!isOverride) {
      state.patient.note.override.used = false;
      state.patient.note.override.reason = '';
      elements.overrideReason.value = '';
    }
    state.patient.note.lastUpdated = `${formatTimestamp(new Date())} by Morgan Lee`;
    elements.noteUpdated.textContent = state.patient.note.lastUpdated;
    if (candidate.evaluation.status === 'confirm') {
      showToast(`${candidate.facility.name} selected — ready to finalize when packet prepared.`);
    } else if (candidate.evaluation.status === 'warn') {
      showToast(`${candidate.facility.name} selected with warnings. Document mitigation before finalize.`);
    } else if (candidate.evaluation.status === 'block' && isOverride) {
      showToast(`${candidate.facility.name} selected with override.`);
    }
    renderPatientTable();
    updateFinalizeAvailability();
    elements.facilityModal.classList.add('hidden');
  ...
  function toggleTab(tabId) {
    // Hide all tabs
    document.getElementById('noteTab').hidden = tabId !== 'noteTab';
    document.getElementById('finderTab').hidden = tabId !== 'finderTab';
    document.getElementById('activityTab').hidden = tabId !== 'activityTab';
    
    // Update tab active state
    document.querySelectorAll('.workflow-nav li').forEach(li => {
  ...
  function addActivity(action, outcome, notes) {
    const entry = {
      action,
      outcome,
      notes,
    ...
    state.patient.activities.unshift(entry);
    renderActivity();
  }

  function addAuditEntry(type, detail) {
    state.patient.audit.push({ type, detail, timestamp: formatTimestamp(new Date()) });
    renderAudit();
  }

  function addPlacementTask(text, owner, badge = 'warn') {
    state.patient.placementPoolTasks.push({ text, owner, badge, due: formatTimestamp(new Date(new Date().getTime() + 30 * 60000)) });
    renderTasks();
  }

  function handleQuickUpdateSave() {
    state.patient.note.matNeeds = elements.matNeeds.value;
    state.patient.note.placementNeeded = elements.placementNeeded.value;
    state.patient.note.requires302 = elements.threeOhTwo.value === '302Required';
    state.patient.note.medicalAcuity = elements.acuity.value;
    state.patient.note.reassessFlag = elements.reassessFlag.value;
    state.patient.note.transferStatus = elements.transferStatus.value;
    if (elements.overrideReason.value.trim()) {
      state.patient.note.override.reason = elements.overrideReason.value.trim();
    }
    if (state.patient.note.reassessFlag !== 'Y') {
      toggleStoryboard(false);
    }
    state.patient.note.lastUpdated = `${formatTimestamp(new Date())} by Morgan Lee`;
    elements.noteUpdated.textContent = state.patient.note.lastUpdated;
    renderFilters();
    ...
    renderPatientTable();
    updateFinalizeAvailability();
    addActivity('Quick update', 'Saved', elements.quickNotes.value.trim() || 'Fields synced to SDE');
    elements.quickNotes.value = '';
    showToast('Quick Update saved — SDEs synced to operational store.');
  }

  function simulateToxResult() {
    state.patient.note.reassessFlag = 'Y';
    state.patient.reassessReasons.push('Benzo screen posted 09:12 after assessment.');
    elements.reassessFlag.value = 'Y';
    toggleStoryboard(true);
    addPlacementTask('Review new benzo result before finalize', 'Placement Pool');
    addAuditEntry('UDS trigger', 'Benzo positive posted after assessment');
    elements.reassessModal.classList.remove('hidden');
    updateFinalizeAvailability();
    showToast('New toxicology posted. Reassessment required.');
  }

  function processReassessResponse(now) {
    elements.reassessModal.classList.add('hidden');
    if (now) {
      addActivity('Reassessment', 'Started', 'Navigator opening Running Note reassessment block.');
    } else {
      const reason = prompt('Log reason for deferring reassessment:') || 'Deferred';
      addActivity('Reassessment', 'Deferred', reason);
      addPlacementTask('Complete reassessment (benzo)', 'CRC RN');
    }
  }

  function handleDismissBanner() {
    if (!elements.overrideReason.value.trim()) {
      showToast('Provide override reason to clear reassessment.');
      elements.overrideReason.focus();
      return;
    }
    state.patient.note.reassessFlag = 'N';
    elements.reassessFlag.value = 'N';
    toggleStoryboard(false);
    addAuditEntry('Override', `Reassess override: ${elements.overrideReason.value.trim()}`);
    updateFinalizeAvailability();
    showToast('Override documented. Reassess flag cleared.');
  }

  function openPacketModal() {
    const facilityName = state.patient.note.selectedFacilityId ? lookupFacilityName(state.patient.note.selectedFacilityId) : 'No facility selected';
    const withheld = !state.patient.consent.act148 || (!state.patient.consent.part2 && !state.breakGlassGranted);
    const withheldSection = withheld ? '\n*** Privacy Notice: HIV/SUD details withheld per consent. ***\n' : '\nHIV/SUD summary: Negative disclosures shared per consent.\n';
    const header = `Recipient: ${facilityName}\nPatient: ${state.patient.name} (MRN ${state.patient.mrn})\nASAM Level: ${state.patient.note.asam}\nMAT Need: ${matLabel(state.patient.note.matNeeds)}\n`;
    const body = `Placement summary: ${state.patient.handoff}\nLast update: ${state.patient.note.lastUpdated}\nFacility decision status: ${state.patient.note.facilityStatus || 'Pending'}\n${withheldSection}`;
    const footer = state.patient.note.transferStatus === 'sent' ? '\nPacket status: SENT' : '\nPacket status: DRAFT';
    elements.packetPreview.textContent = header + '\n' + body + footer;
    elements.packetModal.classList.remove('hidden');
    updateFinalizeAvailability();
  }

  function finalizeTransfer() {
    if (!canFinalize()) {
      showToast('Complete reassessment or select a valid facility before finalizing.');
      return;
    }
    state.patient.note.transferStatus = 'accepted';
    elements.transferStatus.value = 'accepted';
    state.patient.timelineIndex = Math.max(state.patient.timelineIndex, state.patient.timeline.length - 1);
    renderTimeline();
    state.patient.note.lastUpdated = `${formatTimestamp(new Date())} by Morgan Lee`;
    elements.noteUpdated.textContent = state.patient.note.lastUpdated;
    renderPatientTable();
    addActivity('Finalize transfer', 'Completed', `Packet accepted by ${lookupFacilityName(state.patient.note.selectedFacilityId)}.`);
    showToast('Transfer finalized — patient handed off to receiving facility.');
    elements.packetModal.classList.add('hidden');
    updateFinalizeAvailability();
  }

  function markPacketSent() {
    state.patient.note.transferStatus = 'sent';
    elements.transferStatus.value = 'sent';
    state.patient.timelineIndex = Math.min(state.patient.timelineIndex + 1, state.patient.timeline.length - 1);
    renderTimeline();
    addActivity('Packet sent', 'Completed', `Packet transmitted to ${lookupFacilityName(state.patient.note.selectedFacilityId) || 'selected facility'}.`);
    showToast('Packet sent — awaiting facility response.');
  }

  function logCall() {
    const note = prompt('Document call outcome:', 'Spoke with intake; reviewing packet now.');
    state.patient.contact.last = `${formatTimestamp(new Date())} — ${note || 'Call logged'}`;
    elements.contactLast.textContent = state.patient.contact.last;
    addActivity('Call facility', 'Documented', note || 'Call logged');
  }

  function handleActivitySubmit(event) {
    event.preventDefault();
    addActivity(elements.activityAction.value, elements.activityOutcome.value, elements.activityNotes.value.trim());
    elements.activityNotes.value = '';
    showToast('Activity logged.');
  }

  function openBreakGlass() {
    elements.breakGlassModal.classList.remove('hidden');
  }

  function processBreakGlass(event) {
    event.preventDefault();
    state.breakGlassGranted = true;
    state.patient.breakGlassGranted = true;
    const detail = `${elements.breakReason.value} — TTL ${elements.breakDuration.value} mins${elements.breakSupervisor.checked ? '; supervisor notified' : ''}`;
    addAuditEntry('Break-the-Glass', detail);
    elements.breakGlassModal.classList.add('hidden');
    renderPatientContext();
    showToast('Temporary Part 2 access granted and audited.');
  }

  function processScan(event) {
    event.preventDefault();
    state.patient.consent.act148 = elements.scanAct148.checked;
    state.patient.consent.part2 = elements.scanSUD.checked;
    state.patient.consent.expiry = elements.scanExpiry.value || null;
    addAuditEntry('ROI scan', `Act148=${elements.scanAct148.checked} Part2=${elements.scanSUD.checked}`);
    elements.scanModal.classList.add('hidden');
    renderPatientContext();
    showToast('ROI processed. Packet preview updated.');
  }

  function collectSearchForm() {
    searchState.facilities = getSelectedOptions(elements.searchFacilitiesSelect).map(Number).filter((id) => !Number.isNaN(id));
    searchState.overrides = {
      takes302: elements.searchTakes302.checked,
      acuteMed: elements.searchAcuteMed.checked,
      acuteBh: elements.searchAcuteBh.checked,
      secureBh: elements.searchSecureBh.checked,
      dualDx: elements.searchDualDx.checked,
      bhMedInit: elements.searchBhMed.checked,
      inNetwork: elements.searchInNetwork.checked,
    };
    searchState.channels = {
      fax: elements.searchChannelFax.checked,
      direct: elements.searchChannelDirect.checked,
      emr: elements.searchChannelEmr.checked,
    };
    searchState.notes = elements.searchNotesField.value.trim();
    searchState.facilities = Array.from(new Set(searchState.facilities));
  }
  ...
  function renderSearchHistory() {
    if (!elements.searchHistory || !state.patient) return;
    const history = state.patient.searchHistory || [];
    if (!history.length) {
      elements.searchHistory.innerHTML = '<p class="hint">No placement searches published yet.</p>';
      return;
    }
    elements.searchHistory.innerHTML = history
      .map((entry) => {
        const facilityList = entry.facilities.join(', ');
        const channels = entry.channels.join(', ');
        const notes = entry.notes ? ` — ${entry.notes}` : '';
        return `<div class="search-history-entry"><div><strong>${entry.timestamp}</strong> • ${facilityList}</div><div>Channels: ${channels}${notes}</div></div>`;
      })
      .join('');
  }

  function calculateStalenessBadge(search) {
    if (!search.updatedTs) return '';
    const updated = new Date(search.updatedTs.replace(' ', 'T'));
    if (Number.isNaN(updated.getTime())) return '';
    const hours = (Date.now() - updated.getTime()) / (1000 * 60 * 60);
    if (hours > 24) {
      return '<span class="status-chip status-denied">⚠ Aging &gt;24h</span>';
    }
    if (hours > 4) {
      return '<span class="status-chip status-awaiting">⧗ Check back</span>';
    }
    return '';
  }
  ...
  function handleSearchAction(searchId, action) {
    ...
    if (action === 'edit') {
      editSearchEntry(search);
      return;
    }

    if (action === 'delete') {
      if (confirm(`Delete search for ${search.facilityName}?`)) {
    ...
    if (action === 'verify') {
      state.facilityCandidate = { facility: facilities.find((f) => f.id === search.facilityId), evaluation: evaluateFacility(facilities.find((f) => f.id === search.facilityId), patient) };
      openFacilityDetails(state.facilityCandidate.facility, state.facilityCandidate.evaluation);
      return;
    }
  ...
  function appendSearchHistory(search, message) {
    search.history = search.history || [];
    search.history.push({ ts: formatTimestamp(new Date()), message });
  }

  function updateSearchStatus(search, status, reason, extra = {}) {
    const config = statusConfig[status] || statusConfig.Searching;
    search.status = status;
    search.statusReason = reason || '';
    search.updatedTs = formatTimestamp(new Date());
    Object.assign(search, extra);
    appendSearchHistory(search, `${config.label}${reason ? ` — ${reason}` : ''}`);
    renderSearchList();
  }

  function editSearchEntry(searchId) {
    const searchIndex = patient.searches.findIndex(s => s.id === searchId);
    if (searchIndex === -1) {
      alert('Search not found');
      return;
    }

    const search = patient.searches[searchIndex];
    
    // Create edit modal content
    const modalContent = `
      <div class="edit-search-modal">
        <h3>Edit Search: ${search.facilityName}</h3>
        <div class="form-group">
          <label>Search Notes:</label>
          <textarea id="edit-search-notes" rows="3">${search.notes || ''}</textarea>
        </div>
        <div class="form-group">
          <label>Channel:</label>
          <select id="edit-search-channel">
            <option value="phone" ${search.channel === 'phone' ? 'selected' : ''}>Phone</option>
            <option value="email" ${search.channel === 'email' ? 'selected' : ''}>Email</option>
            <option value="fax" ${search.channel === 'fax' ? 'selected' : ''}>Fax</option>
            <option value="portal" ${search.channel === 'portal' ? 'selected' : ''}>Portal</option>
          </select>
        </div>
        <div class="form-group">
          <label>Status:</label>
          <select id="edit-search-status">
            <option value="searching" ${search.status === 'searching' ? 'selected' : ''}>Searching</option>
            ...
            <option value="denied" ${search.status === 'denied' ? 'selected' : ''}>Denied</option>
            <option value="nobeds" ${search.status === 'nobeds' ? 'selected' : ''}>No Beds</option>
            <option value="canceled" ${search.status === 'canceled' ? 'selected' : ''}>Canceled</option>
          </select>
        </div>
        <div class="modal-actions">
          <button onclick="saveSearchEdit('${searchId}')" class="btn-primary">Save Changes</button>
  ...
  function saveSearchEdit(searchId) {
    const searchIndex = patient.searches.findIndex(s => s.id === searchId);
    if (searchIndex === -1) return;

    const search = patient.searches[searchIndex];
    const notes = document.getElementById('edit-search-notes').value;
    const channel = document.getElementById('edit-search-channel').value;
    const status = document.getElementById('edit-search-status').value;
    
    // Update search object
    search.notes = notes;
    search.channel = channel;
    search.status = status;
    search.lastModified = new Date().toISOString();

    // Log the edit
    appendSearchHistory(search, `Search edited - Status: ${status}, Channel: ${channel}`);
  ...
  function deleteSearchEntry(searchId) {
    ...
    // Confirm deletion
    if (confirm(`Are you sure you want to delete the search for ${search.facilityName}?`)) {
      // Log the deletion
      appendSearchHistory(search, `Search deleted for ${search.facilityName}`);
      
      // Remove from searches array
      patient.searches.splice(searchIndex, 1);
  ...
  function populateAcceptanceModal(search) {
    elements.acceptFacilityName.value = search.facilityName;
    elements.acceptUnit.value = search.acceptance?.unit || '';
    elements.acceptBedHold.value = search.acceptance?.bedHold || '';
    elements.acceptClinician.value = search.acceptance?.clinician || '';
    elements.acceptClinicianRole.value = search.acceptance?.clinicianRole || '';
    elements.acceptClinicianNpi.value = search.acceptance?.clinicianNpi || '';
    elements.acceptRep.value = search.acceptance?.rep || '';
    elements.acceptRepRole.value = search.acceptance?.repRole || '';
    elements.acceptRepPhone.value = search.acceptance?.repPhone || '';
    elements.acceptReference.value = search.acceptance?.reference || '';
    elements.acceptTransportMode.value = search.acceptance?.transportMode || '';
    elements.acceptTransportVendor.value = search.acceptance?.transportVendor || '';
    elements.acceptPickupTs.value = search.acceptance?.pickupTs || '';
    elements.acceptDestination.value = search.acceptance?.destination || '';
    elements.acceptEta.value = search.acceptance?.eta || '';
    elements.acceptTransportNotes.value = search.acceptance?.transportNotes || '';
    elements.acceptAttachment.value = search.acceptance?.attachment || '';
    elements.acceptCancelOthers.checked = false;
  }

  function closeAcceptanceModal() {
    elements.acceptanceModal.classList.add('hidden');
    activeAcceptanceRecord = null;
  }

  function populateSearchForm() {
    setSelectedOptions(elements.searchFacilitiesSelect, searchState.facilities.map(String));
    elements.searchTakes302.checked = searchState.overrides.takes302;
    elements.searchAcuteMed.checked = searchState.overrides.acuteMed;
    elements.searchAcuteBh.checked = searchState.overrides.acuteBh;
    elements.searchSecureBh.checked = searchState.overrides.secureBh;
    elements.searchDualDx.checked = searchState.overrides.dualDx;
    elements.searchBhMed.checked = searchState.overrides.bhMedInit;
    elements.searchInNetwork.checked = searchState.overrides.inNetwork;
    elements.searchChannelFax.checked = searchState.channels.fax;
    elements.searchChannelDirect.checked = searchState.channels.direct;
    elements.searchChannelEmr.checked = searchState.channels.emr;
    elements.searchNotesField.value = searchState.notes;
    renderSearchChips();
  }

  function searchFacilitiesByPatient() {
    collectSearchForm();
    if (!state.patient) return;
    const patient = state.patient;
    const requiredMat = patient.note.matNeeds;
    searchState.results = facilities
      .filter((facility) => {
        if (!searchState.facilities.length) return true;
        return searchState.facilities.includes(facility.id);
      })
      .map((facility) => ({ facility, evaluation: evaluateFacility(facility, patient) }))
      .sort((a, b) => b.evaluation.score - a.evaluation.score);
    searchState.lastRun = new Date();
    renderSearchResults();
    updateSearchAudit();
  }
  ...
  function renderSearchResults() {
    ...
    if (!searchState.results.length) {
      elements.searchResults.innerHTML = `
        <div class="empty-state">
          <p class="hint">No search results yet.</p>
          <p class="sub-hint">Use the <strong>Facility Finder</strong> tab to browse available facilities, or run <strong>Verify</strong> to see recommended matches.</p>
          <button class="ghost-btn" id="showAllFacilitiesBtn">View All Available Facilities</button>
        </div>
      `;
      
      // Add event listener for the new button
      const showAllBtn = document.getElementById('showAllFacilitiesBtn');
      if (showAllBtn) {
        showAllBtn.addEventListener('click', () => {
          // Switch to Facility Finder tab
          toggleTab('finderTab');
          // Clear any filters to show all facilities
          elements.filterDistance.value = '';
          elements.hideStale.checked = false;
          elements.showBlocks.checked = true;
          renderFacilities();
    ...
    searchState.results.forEach(({ facility, evaluation }) => {
      const item = document.createElement('div');
      item.className = 'search-result';
      item.innerHTML = `<strong>${facility.name}</strong><span>${evaluation.status.toUpperCase()} • Score ${evaluation.score}</span>`;
  ...
  function publishSearches() {
    ...
    const facilityIds = searchState.facilities.length ? searchState.facilities : [];
    const channelLabels = { fax: 'Fax', direct: 'Secure Email', emr: 'EMR Clinicals' };
    const activeChannels = Object.entries(searchState.channels)
      .filter(([, value]) => value)
      .map(([key]) => channelLabels[key]);
    
    const channelsObj = {
      fax: searchState.channels.fax || false,
      direct: searchState.channels.direct || false,
      emr: searchState.channels.emr || false
    };
    
    const timestamp = formatTimestamp(new Date());
    const bulkId = `BULK-${Date.now()}`;
    
    // Create individual search entries for each facility
    if (facilityIds.length > 0) {
      facilityIds.forEach((facilityId) => {
        const facility = facilities.find((f) => f.id === facilityId);
        if (facility) {
          const searchId = `S-${Date.now()}-${Math.random().toString(36).substr(2, 4)}`;
          const searchEntry = {
            id: searchId,
            ...
            bulkCreateId: bulkId,
            history: [
              { ts: timestamp, user: 'Morgan Lee', action: 'created', message: `Search initiated for ${facility.name}` }
            ]
          };
          patient.searches.push(searchEntry);
        }
      });
    } else {
      // If no specific facilities selected, create a general search
      const searchId = `S-${Date.now()}-${Math.random().toString(36).substr(2, 4)}`;
      const searchEntry = {
        id: searchId,
        facilityId: 'ALL',
        facilityName: 'All recommended facilities',
        status: 'Searching',
        channels: channelsObj,
        notes: searchState.notes || '',
        createdBy: 'Morgan Lee (CRC Tech)',
        ...
        bulkCreateId: bulkId,
        history: [
          { ts: timestamp, user: 'Morgan Lee', action: 'created', message: 'General placement search initiated' }
        ]
      };
      patient.searches.push(searchEntry);
    }
    
    // Create search history entry for audit trail
    const facilityNames = facilityIds.length
      ? facilityIds.map((id) => (facilities.find((f) => f.id === id) || { name: `Facility ${id}` }).name)
      : ['All recommended'];
    ...
    patient.searchHistory.unshift({
      ...
      channels: activeChannels.length ? activeChannels : ['None selected'],
      notes: searchState.notes,
    });
    
    addActivity('Placement search', 'Published', `${facilityNames.join(', ')}; channels: ${activeChannels.length ? activeChannels.join(', ') : 'none'}.`);
    patient.note.lastUpdated = `${timestamp} by Morgan Lee`;
    elements.noteUpdated.textContent = patient.note.lastUpdated;
    
    // Save data using persistence system
    savePatientData();
    ...
    renderSearchResults();
    closeSearchPopover();
    showToast(`Created ${facilityIds.length || 1} placement search${facilityIds.length === 1 ? '' : 'es'}.`);
  }

  function renderFilters() {
    populateAsamSelect(elements.filterAsam);
    const asamValue = parseAsamLevel(state.patient.note.asam);
    setSelectedOptions(elements.filterAsam, asamValue ? [asamValue] : []);
    populateMatSelect(elements.filterMat, true);
    elements.filterMat.value = state.patient.note.matNeeds === 'None' ? '' : state.patient.note.matNeeds;
    elements.filterAcuteBh.checked = state.patient.note.medicalAcuity !== 'Routine';
    elements.filterAcuteMed.checked = ['Secured', 'StepUp'].includes(state.patient.note.medicalAcuity);
    elements.filter302.value = state.patient.note.requires302 ? 'required' : '';
    elements.filterPayer.value = state.patient.payer || '';
  }

  function setupEvents() {
    elements.patientSelect.addEventListener('change', () => loadPatient(elements.patientSelect.value));
    ...
    elements.saveQuickUpdate.addEventListener('click', handleQuickUpdateSave);
    elements.postTox.addEventListener('click', simulateToxResult);
    elements.dismissBanner.addEventListener('click', handleDismissBanner);
    elements.simulatePacket.addEventListener('click', openPacketModal);
    elements.closePacket.addEventListener('click', () => elements.packetModal.classList.add('hidden'));
    elements.packetFinalize.addEventListener('click', finalizeTransfer);
    elements.packetPrint.addEventListener('click', () => {
      const limited = !state.patient.consent.act148 || (!state.patient.consent.part2 && !state.breakGlassGranted);
      showToast(limited ? 'Print group: No-HIV/No-SUD packet generated.' : 'Print group: Standard full packet generated.');
    });
    elements.logCallAttempt.addEventListener('click', logCall);
    elements.markPacketSent.addEventListener('click', markPacketSent);
    elements.scanConsent.addEventListener('click', () => {
      elements.scanAct148.checked = state.patient.consent.act148;
      elements.scanSUD.checked = state.patient.consent.part2;
      elements.scanExpiry.value = state.patient.consent.expiry || '';
      elements.scanModal.classList.remove('hidden');
    });
    elements.closeScan.addEventListener('click', () => elements.scanModal.classList.add('hidden'));
    elements.scanForm.addEventListener('submit', processScan);
    elements.matNeeds.addEventListener('change', () => {
      if (elements.matNeeds.value && elements.matNeeds.value !== 'None') {
        elements.filterMat.value = elements.matNeeds.value;
      } else {
        elements.filterMat.value = '';
      }
      renderFacilities();
    });
    elements.facilitySearch.addEventListener('input', handleSearchInput);
    ...
    elements.threeOhTwo.addEventListener('change', renderFacilities);
    elements.acuity.addEventListener('change', () => {
      elements.filterAcuteBh.checked = elements.acuity.value !== 'Routine';
      elements.filterAcuteMed.checked = ['Secured', 'StepUp'].includes(elements.acuity.value);
      renderFacilities();
    });
    elements.filterAsam.addEventListener('change', renderFacilities);
    elements.filterMat.addEventListener('change', renderFacilities);
    elements.filter302.addEventListener('change', renderFacilities);
    elements.filterAcuteBh.addEventListener('change', renderFacilities);
    elements.filterAcuteMed.addEventListener('change', renderFacilities);
    elements.filterPayer.addEventListener('change', renderFacilities);
    elements.filterDistance.addEventListener('input', renderFacilities);
    elements.hideStale.addEventListener('change', renderFacilities);
    elements.showBlocks.addEventListener('change', renderFacilities);
    elements.activityForm.addEventListener('submit', handleActivitySubmit);
    elements.clearActivity.addEventListener('click', () => elements.activityNotes.value = '');
    elements.advanceTimeline.addEventListener('click', () => {
      state.patient.timelineIndex = Math.min(state.patient.timelineIndex + 1, state.patient.timeline.length - 1);
      renderTimeline();
    });
    elements.resetTimeline.addEventListener('click', () => {
      state.patient.timelineIndex = 0;
      renderTimeline();
    });
    elements.openDirectory.addEventListener('click', () => showToast('Directory admin prototype launching (not implemented).'));
    elements.exportAudit.addEventListener('click', () => showToast('Audit export queued for Privacy review.'));
    elements.breakGlassBtn.addEventListener('click', openBreakGlass);
    elements.closeBreakGlass.addEventListener('click', () => elements.breakGlassModal.classList.add('hidden'));
    elements.breakGlassForm.addEventListener('submit', processBreakGlass);
    elements.closeFacilityDetails.addEventListener('click', () => elements.facilityDetailsModal.classList.add('hidden'));
    elements.detailsSelect.addEventListener('click', () => {
      if (!state.facilityCandidate) return;
      const evaluation = evaluateFacility(state.facilityCandidate.facility, state.patient);
      elements.facilityDetailsModal.classList.add('hidden');
      openFacilityModal(state.facilityCandidate.facility, evaluation);
    });
    elements.detailsCall.addEventListener('click', () => {
      if (!state.facilityCandidate) return;
      logFacilityCall(state.facilityCandidate.facility);
      elements.facilityDetailsModal.classList.add('hidden');
    });
    elements.addSearchBtn.addEventListener('click', () => {
      ensureSearchArrays(state.patient);
      populateSearchFacilityOptions();
      resetSearchState();
      openSearchPopover();
    });
    elements.addMultipleSearchBtn.addEventListener('click', () => {
      ensureSearchArrays(state.patient);
      populateSearchFacilityOptions();
      resetSearchState();
      openSearchPopover();
      showToast('Bulk placement search prototype — configure per facility manually.');
    });
    elements.closeSearchPopover.addEventListener('click', closeSearchPopover);
    elements.searchCancel.addEventListener('click', closeSearchPopover);
    elements.searchAddOther.addEventListener('click', (event) => {
      event.preventDefault();
      showToast('Use directory steward tooling to add facilities.');
    });
    elements.searchVerify.addEventListener('click', () => {
      searchFacilitiesByPatient();
      showToast('Verify run complete — suggested facilities refreshed.');
    });
    elements.searchPublish.addEventListener('click', () => {
      publishSearches();
      renderSearchHistory();
      renderSearchList();
    });
    elements.closeAcceptanceModal.addEventListener('click', closeAcceptanceModal);
    elements.acceptanceForm.addEventListener('submit', (event) => {
      event.preventDefault();
      if (!activeAcceptanceRecord || !state.patient) {
        closeAcceptanceModal();
        return;
      }
      const acceptance = {
        unit: elements.acceptUnit.value.trim(),
        bedHold: elements.acceptBedHold.value,
        clinician: elements.acceptClinician.value.trim(),
        clinicianRole: elements.acceptClinicianRole.value.trim(),
        clinicianNpi: elements.acceptClinicianNpi.value.trim(),
        rep: elements.acceptRep.value.trim(),
        repRole: elements.acceptRepRole.value.trim(),
        repPhone: elements.acceptRepPhone.value.trim(),
        reference: elements.acceptReference.value.trim(),
        transportMode: elements.acceptTransportMode.value.trim(),
        transportVendor: elements.acceptTransportVendor.value.trim(),
        pickupTs: elements.acceptPickupTs.value,
        destination: elements.acceptDestination.value.trim(),
        eta: elements.acceptEta.value,
        transportNotes: elements.acceptTransportNotes.value.trim(),
        attachment: elements.acceptAttachment.value.trim()
      };
      activeAcceptanceRecord.acceptance = acceptance;
      updateSearchStatus(activeAcceptanceRecord, 'Accepted', 'Facility accepted placement', acceptance);
      applyAcceptanceToPatient(state.patient, activeAcceptanceRecord, acceptance, elements.acceptCancelOthers.checked);
      closeAcceptanceModal();
      renderSearchHistory();
      renderSearchList();
      renderPatientTable();
    ...
    elements.addMultipleSearchBtn.addEventListener('click', () => {
      ...
      populateSearchForm();
      openSearchPopover();
      showToast('Bulk placement search prototype — configure overrides, then Publish.');
    });
    elements.closeSearchPopover.addEventListener('click', closeSearchPopover);
    elements.searchCancel.addEventListener('click', () => {
      closeSearchPopover();
    });
    elements.searchFacilitiesSelect.addEventListener('change', () => {
      collectSearchForm();
      setSelectedOptions(elements.searchFacilitiesSelect, searchState.facilities.map(String));
      renderSearchChips();
    });
    elements.searchAddOther.addEventListener('click', (event) => {
      event.preventDefault();
      showToast('Use Directory Steward console to add additional facilities.');
    });
    [
      elements.searchTakes302,
      elements.searchAcuteMed,
      elements.searchAcuteBh,
      elements.searchSecureBh,
      elements.searchDualDx,
      elements.searchBhMed,
      elements.searchInNetwork,
      elements.searchChannelFax,
      elements.searchChannelDirect,
      elements.searchChannelEmr
    ].forEach((input) => input.addEventListener('change', () => {
      collectSearchForm();
    }));
    elements.searchNotesField.addEventListener('input', () => {
      collectSearchForm();
    });
    elements.searchVerify.addEventListener('click', () => {
      searchFacilitiesByPatient();
      updateSearchAudit();
      showToast('Verify run complete — suggested facilities updated.');
    });
    elements.searchPublish.addEventListener('click', () => {
      publishSearches();
      renderSearchHistory();
    ...
    elements.closeFacilityModal.addEventListener('click', closeFacilityModal);
    elements.closeReassess.addEventListener('click', () => processReassessResponse(false));
    elements.startReassess.addEventListener('click', () => processReassessResponse(true));
    elements.deferReassess.addEventListener('click', () => processReassessResponse(false));
    elements.finalizeButton.addEventListener('click', finalizeTransfer);
    elements.openDowntime.addEventListener('click', () => showToast('Downtime playbook downloaded to Desktop.'));
    
    // Active Searches link handler
    const showActiveSearchesLink = document.getElementById('showActiveSearches');
    console.log('Active Searches Link:', showActiveSearchesLink);
    if (showActiveSearchesLink) {
      showActiveSearchesLink.addEventListener('click', (event) => {
        console.log('Active Searches clicked!');
        event.preventDefault();
        // Switch to Running Note tab if not already active
        elements.navTabs.forEach((tab) => tab.classList.remove('active'));
        const noteTab = document.querySelector('[data-tab="noteTab"]');
        if (noteTab) {
          noteTab.classList.add('active');
        }
        toggleTab('noteTab');
        
        // Scroll to the Active Placement Searches section
        setTimeout(() => {
          const searchListElement = document.getElementById('searchList');
          console.log('Search List Element:', searchListElement);
          if (searchListElement) {
            searchListElement.scrollIntoView({ 
              behavior: 'smooth', 
              block: 'start' 
            });
            // Add a subtle highlight effect
            searchListElement.style.outline = '2px solid var(--epic-primary)';
            searchListElement.style.outlineOffset = '4px';
            setTimeout(() => {
              searchListElement.style.outline = '';
              searchListElement.style.outlineOffset = '';
            }, 2000);
            showToast('📋 Navigated to Active Placement Searches', 'success');
          } else {
            showToast('❌ Search list not found', 'error');
          }
        }, 100);
      });
    } else {
      console.error('Active Searches button not found!');
    }
    
    // Settings link handler
    const openSettingsLink = document.getElementById('openSettings');
    console.log('Settings Link:', openSettingsLink);
    if (openSettingsLink) {
      openSettingsLink.addEventListener('click', (event) => {
        console.log('Settings clicked!');
        event.preventDefault();
        // Show settings modal or open in new tab
        const settingsUrl = '/Users/VScode_Projects/EMR/Review/settings.md';
        showToast('📋 Settings: Opening configuration documentation...', 'info');
        
        // Try to open the settings file
        try {
          // For a web application, we'll show the settings content in a modal
          openSettingsModal();
        } catch (error) {
          console.log('Opening settings file:', settingsUrl);
          showToast('⚙️ Settings file location: /Users/VScode_Projects/EMR/Review/settings.md', 'info');
        }
      });
    } else {
    ...
    window.addEventListener('keydown', (event) => {
      if (event.key === 'Escape') {
        [elements.facilityModal, elements.packetModal, elements.breakGlassModal, elements.reassessModal, elements.scanModal, elements.searchPopover].forEach((modal) => modal.classList.add('hidden'));
      }
    });
    
    // Add click-outside-to-close for search popover
    elements.searchPopover.addEventListener('click', (event) => {
      if (event.target === elements.searchPopover) {
        closeSearchPopover();
      }
    });
  }
  ...
  function loadPatient(patientId) {
    ...
    state.breakGlassGranted = patient.breakGlassGranted || false;
    state.selectedFacility = patient.note.selectedFacilityId ? facilities.find((fac) => fac.id === patient.note.selectedFacilityId) : null;
    state.facilityCandidate = null;
    elements.patientSelect.value = patient.id;
    ensureSearchArrays(patient); // Ensure search arrays exist
    if (!patient.searchHistory) patient.searchHistory = [];
    populateSearchFacilityOptions();
    resetSearchState();
    renderPatientContext();
    ...
    renderFilters();
    renderSearchHistory();
    renderSearchList(); // Add this to show active searches on Running Note
    elements.filterDistance.value = '';
    state.searchTerm = '';
    elements.facilitySearch.value = '';
    elements.clearSearch.disabled = true;
  ...
  // ============================================
  // Patient List Preview Functions
  // ============================================

  function renderPatientList(filteredData = null) {
    const tbody = elements.patientListBody || document.getElementById('patientListBody');
    if (!tbody) return;

    const data = filteredData || patientListData;
    tbody.innerHTML = '';
  ...
  function createPatientRow(patient) {
    ...
    tr.className = `plac-${patient.status}`;
    ...
    tr.innerHTML = `
      ...
      <td class="col-placement" data-placement-type="${patient.placement.type}">
        ${renderPlacementCell(patient.placement)}
      </td>
      ...
      <td class="col-transport">
        ${renderTransportCell(patient.transport)}
      </td>
      <td class="col-assigned">
        ${renderAssignedCell(patient.assigned)}
      </td>
      <td class="col-lastupdate">${patient.lastUpdate}</td>
      <td class="col-reassess reassess-flag">
        <span class="${patient.reassess ? 'reassess-yes' : 'reassess-no'}">
          ${patient.reassess ? 'Yes' : 'No'}
        </span>
      </td>
    `;
    ...
    // Add click handlers
    setupPatientRowEvents(tr, patient);

    return tr;
  }

  function renderPlacementCell(placement) {
    if (placement.type === 'placed') {
      return `
        <div class="placement-primary">${placement.facilityName}</div>
        <div class="placement-secondary">${placement.unit} • Bed hold until ${placement.bedHold}</div>
      `;
    } else {
      return `<span class="asam-tag">${placement.asamLevel}</span>`;
    }
  }
  ...
  function renderStatusBadge(status) {
    const statusConfig = {
      ...
      waiting: { label: 'Transport', icon: '⌛', class: 'badge-waiting' },
      nobeds: { label: 'No Beds', icon: '∅', class: 'badge-nobeds' },
      denied: { label: 'Denied', icon: '⨯', class: 'badge-denied' },
      searching: { label: 'Searching', icon: '…', class: 'badge-searching' },
      canceled: { label: 'Canceled', icon: '', class: 'badge-canceled' }
    };
  ...
  function renderTransportCell(transport) {
    switch (transport.status) {
      case 'waiting':
        return `
          <div class="transport-status">
            <span class="transport-dot waiting"></span>
            Waiting
          </div>
        `;
      case 'scheduled':
        return `
          <div class="transport-status">
            <span class="transport-dot scheduled"></span>
            Scheduled (${transport.eta})
          </div>
        `;
      case 'complete':
        return `
          <div class="transport-status">
            <span class="transport-dot complete"></span>
            Complete
          </div>
        `;
      default:
  ...
  function renderAssignedCell(assigned) {
    if (!assigned || !assigned.name) {
      return '<span class="assigned-staff">—</span>';
    }
    
    return `
      <div class="assigned-staff" data-staff-id="${assigned.id}">
        ${assigned.name}
        <div class="staff-role">${assigned.role}</div>
      </div>
    `;
  }

  function setupPatientRowEvents(row, patient) {
    // Placement cell click
    const placementCell = row.querySelector('[data-placement-type]');
    if (placementCell) {
      placementCell.addEventListener('click', (e) => {
        e.stopPropagation();
        handlePlacementClick(patient);
      });
    }
  ...
  function applyPatientListFilter(filterType, filterValue = null) {
    ...
    switch (filterType) {
      case 'placed':
        filteredData = filteredData.filter(p => p.placement.type === 'placed');
        break;
      case 'transport':
        filteredData = filteredData.filter(p => 
          p.transport.status === 'waiting' || p.transport.status === 'scheduled'
        );
        break;
      case 'status':
        if (filterValue) {
  ...
  function filterSearchList(searches) {
    return searches.filter(search => {
      ...
      // Status filter
      if (searchListFilters.status && search.status !== searchListFilters.status) {
        return false;
      }

      // Facility filter
      if (searchListFilters.facility && search.facilityId !== searchListFilters.facility) {
        return false;
      }
  ...
  function handleQuickAction(searchId, action) {
    ...
    switch (action) {
      case 'edit':
        openEditSearchModal(search);
        break;
      case 'details':
        openSearchDetailsModal(search);
        break;
      case 'delete':
        if (confirm(`Delete search for ${search.facilityName}?`)) {
  ...
  function openEditSearchModal(search) {
    // Use the Epic modal system
    window.openEpicModal(search);
  }

  function openSearchDetailsModal(search) {
    // Use the Epic details system
    window.showSearchDetails(search.id);
  }

  function deleteSearch(searchId) {
    if (!state.patient?.searches) return;
    
    state.patient.searches = state.patient.searches.filter(s => s.id !== searchId);
    renderFilteredSearchList();
  ...
  function filterSearchList(searches) {
    return searches.filter(search => {
      ...
      if (searchListFilters.facility && !search.facilityName.toLowerCase().includes(searchListFilters.facility.toLowerCase())) {
        return false;
      }
      return true;
    });
  ...
  function createPatientRow(patient) {
    ...
    tr.innerHTML = `
      ...
      <td class="col-mat">
        <span class="mat-need">${patient.mat}</span>
      </td>
      <td class="col-status">
        ${renderStatusBadge(patient.status)}
      </td>
      ...
      <td class="col-lastupdate">
        <span class="last-update">${patient.lastUpdate}</span>
      </td>
      <td class="col-reassess">
        ${patient.reassess ? '<span class="reassess-flag">⚠️</span>' : '—'}
      </td>
    `;
  ...
  function setupPatientListFilters() {
    ...
    const filterChips = document.querySelectorAll('.patient-list-filters .filter-chip');
    filterChips.forEach(chip => {
      chip.addEventListener('click', () => {
        filterChips.forEach(c => c.classList.remove('active'));
        chip.classList.add('active');
    ...
    if (statusFilter) {
      statusFilter.addEventListener('change', applyPatientListFilters);
    }
    if (assignedFilter) {
      assignedFilter.addEventListener('change', applyPatientListFilters);
    }
  }

  function applyPatientListFilter(filterType) {
    let filteredData = [...patientListData];
    ...
    switch (filterType) {
      case 'placed':
        ...
        break;
      case 'transport':
        filteredData = filteredData.filter(p => p.transport.status !== 'not-required');
        break;
      case 'all':
      default:
        // Show all
  ...
  function applyPatientListFilters() {
    ...
    if (statusFilter && statusFilter.value) {
      filteredData = filteredData.filter(p => p.status === statusFilter.value);
    }

    if (assignedFilter && assignedFilter.value) {
      if (assignedFilter.value === 'unassigned') {
        filteredData = filteredData.filter(p => !p.assigned || !p.assigned.id);
      } else {
        filteredData = filteredData.filter(p => p.assigned && p.assigned.id === assignedFilter.value);
      }
    }
      ...
      if (savedData) {
        ...
        if (parsedData.patients && Array.isArray(parsedData.patients)) {
          ...
          parsedData.patients.forEach((savedPatient, index) => {
            if (patients[index] && patients[index].id === savedPatient.id) {
              // Update searches and other dynamic data
              patients[index].searches = savedPatient.searches || [];
              patients[index].searchHistory = savedPatient.searchHistory || [];
              patients[index].note = { ...patients[index].note, ...savedPatient.note };
              patients[index].activities = savedPatient.activities || patients[index].activities || [];
            }
          });
  ...
  function init() {
    ...
    // Force initial render of patient list
    setTimeout(() => {
      renderPatientList();
      console.log('Patient List initialized with', patientListData.length, 'patients');
    }, 200);
