...
(() => {
  ...
  const statusConfig = {
    ...
    Expired: { ...canonicalStatusPalette.Waiting, label: 'Hold expired', icon: '⧗', canonical: 'Waiting' },
    ReassessRequired: { ...canonicalStatusPalette.Waiting, label: 'Reassess required', icon: '⚠️', canonical: 'Waiting' }
  };
  ...
  const statusOptions = [
    ...
    { value: 'ReassessRequired', label: 'Reassess required' }
  ];
...
function loadStoredPanelVisibility() {
  ...
  function mapStatusToFilter(status) {
    ...
    if (['Accepted'].includes(key)) return 'accepted';
    if (['Denied'].includes(key)) return 'denied';
    if (['NoBeds'].includes(key)) return 'nobeds';
    if (['Canceled'].includes(key)) return 'canceled';
    if (['WaitingTransport'].includes(key)) return 'waiting';
    if (['PendingReview', 'PacketSent', 'AwaitingCallback', 'ReassessRequired'].includes(key)) return 'pending';
  ...
  function normalizeStatusKey(value = '') {
    ...
    const parts = String(value)
      ...
      .split(/\s+/)
      .filter(Boolean);
    if (!parts.length) return 'Searching';
    const composed = parts
      .map((part) => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase())
      .join('');
    return statusConfig[composed] ? composed : composed;
  }
  ...
  function formatStatusDisplay(statusKey = 'Searching') {
    const config = statusConfig[statusKey] || statusConfig.Searching;
    let label = config.label || statusKey;
    const statusLabelOverrides = {
      Searching: 'Searching',
      PendingReview: 'Pending review',
      PacketSent: 'Packet sent',
      WaitingTransport: 'Waiting transport',
      Accepted: 'Accepted',
      Denied: 'Denied',
      NoBeds: 'No beds',
      Canceled: 'Canceled',
      Expired: 'Hold expired',
      ReassessRequired: 'Reassess required'
    };
    if (statusLabelOverrides[statusKey]) {
      label = statusLabelOverrides[statusKey];
    } else {
      const waitingMatch = label.match(/^Waiting —\s*(.+)$/i);
      if (waitingMatch) {
        const remainder = waitingMatch[1];
        label = `Waiting ${remainder.charAt(0).toLowerCase()}${remainder.slice(1)}`;
      }
    }
  ...
  function mapScenarioToRow(patient) {
    ...
    const statusKey = normalizeStatusKey(board.statusKey || board.statusLabel || 'Searching');
    ...
    return {
      ...
      needsTransport: board.needsTransport || statusKey === 'WaitingTransport'
    };
  }
  ...
  function mapOverrideToRow(entry = {}) {
    ...
    return {
      ...
      needsTransport: statusKey === 'WaitingTransport'
    };
  }
  ...
  function renderPatientList() {
    ...
    dom.patientListBody.innerHTML = sorted
      .map((row) => {
        ...
        const config = statusConfig[key] || statusConfig.Searching;
        const chipClass = config.chipClass || `status-badge badge-${statusKeyToClass(key)}`;
        return `
          <tr data-patient-id="${row.id}">
            <td>${row.name || ''}</td>
            <td>${row.placement || ''}</td>
            <td>${row.mat || ''}</td>
            <td><span class="${chipClass}">${display}</span></td>
            <td>${row.transport || ''}</td>
