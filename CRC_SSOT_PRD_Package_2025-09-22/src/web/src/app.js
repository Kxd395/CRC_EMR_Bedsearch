...
(() => {
  ...
  const statusConfig = {
    ...
    Pending: { ...canonicalStatusPalette.Waiting, label: 'Waiting — Pending', canonical: 'Waiting' }, // Fallback for legacy data
  ...
  let loadedPatients = []; // Patients loaded from database
  let databaseLoadAttempted = false;
  let databaseLoadFailed = false;
  let retryCount = 0;
  const MAX_RETRIES = 3;

  // Patient scenarios using real facility data from update2.md
...
function loadStoredPanelVisibility() {
  ...
  /**
   * Load patients from database with exponential backoff retry
   */
  async function loadPatientsFromDatabase() {
    ...
    try {
      console.log(`%c🔄 Attempting to load patients from database (attempt ${retryCount + 1}/${MAX_RETRIES})...`, 'color: blue');
      ...
      const patients = responseData.data || responseData; // Handle both { data: [...] } and [...] formats
      console.log(`%c✅ DATABASE CONNECTED: Loaded ${patients.length} patients from PostgreSQL`, 'color: green; font-weight: bold');
      console.log(`   → Database: emr_crc_ssot @ 100.112.67.23:5432`);
      console.log(`   → Method: ${responseData.method || 'unknown'}`);
      ...
      console.error(`%c⚠️ DATABASE LOAD FAILED (attempt ${retryCount + 1}):`, 'color: red; font-weight: bold', error.message);
      ...
      if (retryCount < MAX_RETRIES) {
        ...
        setTimeout(() => loadPatientsFromDatabase(), delay);
      } else {
        databaseLoadFailed = true;
        console.log(`%c📋 FALLBACK MODE: Using static patient data only`, 'color: orange; font-weight: bold');
        console.log(`   → All data from: newPatientScenarios.js`);
        console.log(`   → Database unavailable - changes will NOT persist`);
      }
  ...
  async function savePatientToDatabase(patient) {
    try {
      console.log(`💾 Saving patient ${patient.id} to database...`);
      ...
      const responseData = await response.json();
      const saved = responseData.data || responseData; // Handle both { data: {...} } and {...} formats
      console.log(`✅ Patient ${patient.id} saved successfully`);

      // Transform saved patient back to UI format
      const uiPatient = transformPatientFromDatabase(saved);

      // Update loadedPatients array
      const index = loadedPatients.findIndex(p => p.id === patient.id);
      if (index >= 0) {
        loadedPatients[index] = uiPatient;
      } else {
        loadedPatients.push(uiPatient);
      }

      return uiPatient;
    } catch (error) {
      console.error(`❌ Failed to save patient ${patient.id}:`, error.message);
      
      // Don't retry - just log the error
      // The patient data is still in memory and fallback files
  ...
  /**
   * Transform UI patient object to database format
   */
  function transformPatientForDatabase(patient) {
    ...
    return {
      ...
      admissionStatus: patient.board?.statusKey || patient.note?.transferStatus || 'active',
      ...
      insurancePrimary: patient.insurance?.primary || null,
      insuranceSecondary: patient.insurance?.secondary || null,
      emergencyContactName: patient.contact?.name || null,
      emergencyContactPhone: patient.contact?.phone || null,
  ...
  /**
   * Transform database patient object to minimal format for merging
   * 
   * Since we now use mergePatientData() to combine with static data,
   * we only need to extract the fields that actually come from database.
   * No need to build complete UI structure here.
   */
  function transformPatientFromDatabase(dbPatient) {
    // If already transformed, return as-is
    if (dbPatient.searches && !dbPatient.first_name && !dbPatient.bedSearches) {
      return dbPatient;
    }

    // CRITICAL: API returns "bedSearches" but UI expects "searches"
    const searches = dbPatient.searches || dbPatient.bedSearches || [];
    const searchHistory = dbPatient.searchHistory || dbPatient.search_history || [];

    console.log(`🔄 Transforming patient ${dbPatient.id}: ${searches.length} searches found`);

    // Return minimal structure with database fields only
    // mergePatientData() will combine this with static patient
    return {
      id: dbPatient.id,
      
      // Name fields (for potential future use)
      firstName: dbPatient.firstName || dbPatient.first_name,
      lastName: dbPatient.lastName || dbPatient.last_name,
      mrn: dbPatient.mrn,
      
      // Assessment fields that can be updated
      ...
      // Persistence fields (CRITICAL - these are user changes!)
      searches: searches,
      searchHistory: searchHistory,
      
      // Metadata
      updated_at: dbPatient.updated_at || dbPatient.last_updated
    };
  }
  ...
  async function loadPatientAssessment(patientId) {
    try {
      console.log(`📝 Loading assessment data for patient: ${patientId}`);
      
      const response = await fetch(`http://localhost:3001/api/patients/${patientId}/assessments`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' },
        mode: 'cors'
      });
      ...
      if (!response.ok) {
        console.warn(`⚠️ Failed to load assessment for ${patientId}: HTTP ${response.status}`);
        return null;
      }
      ...
      if (responseData.success && responseData.data && responseData.data.length > 0) {
        ...
        console.log(`✅ Loaded assessment data for ${patientId}:`, assessment);
        return assessment;
      }
      
      console.log(`ℹ️ No assessment data found for ${patientId}`);
      return null;
    } catch (error) {
      console.error(`❌ Error loading assessment for ${patientId}:`, error.message);
      return null;
    }
  }

  /**
   * Reload current patient from database to ensure consistency
   */
  async function reloadCurrentPatientFromDatabase() {
    ...
    try {
      ...
      const responseData = await response.json();
      const fresh = responseData.data || responseData; // Handle both { data: {...} } and {...} formats
      
      // Update in loadedPatients
      const index = loadedPatients.findIndex(p => p.id === currentId);
      if (index >= 0) {
        loadedPatients[index] = fresh;
      }

      // Re-render
      renderAll();
      
      console.log(`🔄 Reloaded patient ${currentId} from database`);
      return fresh;
    } catch (error) {
      console.warn(`⚠️ Could not reload patient ${currentId}:`, error.message);
      return null;
    }
  }

  /**
   * Background database reconnection check (every 10 seconds)
   */
  function scheduleBackgroundDatabaseCheck() {
    setInterval(async () => {
      if (databaseLoadFailed || !databaseLoadAttempted) {
        console.log('🔄 Background: Attempting database reconnection...');
        const success = await loadPatientsFromDatabase();
        if (success) {
          console.log('✅ Background: Database reconnected successfully');
          showToast('Database connection restored');
        }
      }
  ...
  function startPeriodicSync() {
    setInterval(async () => {
      if (!databaseLoadFailed && loadedPatients.length > 0) {
        console.log('🔄 Periodic sync: Reloading current patient from database...');
  ...
  function populatePatientSelector() {
    ...
    // Use static patients for selector (they have complete structure with label)
    // getCurrentPatient() will handle merging with database data when needed
    const dbIds = new Set(loadedPatients.map(p => p.id));
    
    // Store current value to restore without triggering change event
    const currentValue = state.currentPatientId;
    
    dom.patientSelect.innerHTML = patientScenarios
      .map((patient) => {
        const isFromDb = dbIds.has(patient.id);
        const prefix = isFromDb ? '💾 ' : '📋 '; // 💾 = has database data, 📋 = static only
        return `<option value="${patient.id}">${prefix}${patient.label}</option>`;
      })
      .join('');
    
    // Set value silently without triggering change event
    // (innerHTML replacement already reset the value, so this is just a safety check)
    if (dom.patientSelect.value !== currentValue) {
      dom.patientSelect.value = currentValue;
    }
  }

  /**
   * Get current patient by merging static data (complete structure) with database updates
   * 
   * Strategy:
   * - Static data provides complete patient structure (identifiers, consent, activities, etc.)
   * - Database provides user changes (searches, updated assessments, etc.)
   * - Merge them together for complete, up-to-date patient
   */
  function getCurrentPatient() {
    ...
    if (!staticPatient) {
      ...
      return patientScenarios[0]; // Fallback to first patient
    }
    
    // Check if we have database updates
    const dbPatient = loadedPatients.find((p) => p.id === currentId);
    if (!dbPatient) {
      // No database data yet, use static as-is
      console.log(`%c📋 STATIC MODE: Using static patient data for ${currentId} (no database record)`, 'color: orange; font-weight: bold');
      console.log(`   → All data from: newPatientScenarios.js`);
      console.log(`   → Changes will NOT persist on refresh`);
      return staticPatient;
    }
    
    // Merge: Static structure + Database updates
    console.log(`%c� DATABASE MODE: Merging static + database data for ${currentId}`, 'color: green; font-weight: bold');
    console.log(`   → Structure from: newPatientScenarios.js (static)`);
    console.log(`   → Searches from: PostgreSQL database (${dbPatient.searches?.length || 0} searches)`);
    console.log(`   → Assessment from: ${dbPatient.asamLevel ? 'Database' : 'Static'}`);
    return mergePatientData(staticPatient, dbPatient);
  }

  /**
   * Merge static patient structure with database updates
   * 
   * @param {Object} staticPatient - Complete patient structure from newPatientScenarios.js
   * @param {Object} dbPatient - Database patient with user changes (flat structure)
   * @returns {Object} Complete patient with database updates applied
   */
  function mergePatientData(staticPatient, dbPatient) {
    return {
      ...staticPatient,  // Start with complete static structure
      
      // Overlay database fields that can change
      searches: dbPatient.searches || staticPatient.searches || [],
      searchHistory: dbPatient.searchHistory || staticPatient.searchHistory || [],
      
      // Update note fields from database (if they exist)
      note: {
        ...staticPatient.note,
  ...
  function renderPatientContext(patient) {
    ...
    // Load assessment data when patient is displayed
    loadAndRenderAssessment(patient.id);
  }

  /**
   * Load and render assessment data for the current patient
   */
  let currentAssessmentLoad = null; // Track current load to prevent race conditions
  let userIsEditingAssessment = false; // Track if user is actively editing
  let isLoadingAssessment = false; // Prevent concurrent loads
  ...
  async function loadAndRenderAssessment(patientId, forceReload = false) {
    ...
    if (assessment) {
      // Populate ASAM field
      if (assessment.asamLevel && dom.asamField) {
        // Set the value directly (value attribute, not label)
        dom.asamField.value = assessment.asamLevel;
      }
      
      // Populate MAT needs
      if (assessment.matNeeds && dom.matNeeds) {
        // Handle both object {type: "MethadoneContinue"} and string "MethadoneContinue"
        const matValue = typeof assessment.matNeeds === 'object' ? assessment.matNeeds.type : assessment.matNeeds;
        dom.matNeeds.value = matValue || '';
      }
      
      // Populate commitment status
      if (assessment.commitmentStatus && dom.threeOhTwo) {
        ...
        dom.threeOhTwo.value = commitmentMap[assessment.commitmentStatus] || assessment.commitmentStatus || '';
      }
      
      // Populate medical acuity
      if (assessment.medicalAcuity && dom.acuity) {
        // Map from backend values to UI values
        const acuityMap = {
          'Medically Monitored': 'StepUp',
          'Routine monitoring': 'Routine',
          'Q4 routine monitoring': 'Q4',
          'Q8 routine monitoring': 'Q8',
          'Q12 routine monitoring': 'Q12',
          'Step-up (IV meds)': 'StepUp',
          'Secure detox': 'Secured'
        };
        dom.acuity.value = acuityMap[assessment.medicalAcuity] || assessment.medicalAcuity || 'Routine';
      }
      
      console.log('✅ Assessment fields populated from API data');
    } else {
      console.log('ℹ️ No assessment data, fields remain empty or show defaults');
    }
    } finally {
  ...
  function renderActiveSearches(patient) {
    ...
    dom.searchList.innerHTML = searches
      .map((search) => {
        ...
        console.log(`🎨 Rendering ${search.facilityName}: status="${search.status}" → ${config.chipClass}`);
  ...
  const documentCodes = [
    { code: 'HIPAA_ACK', label: 'HIPAA Acknowledgment', required: true },
    { code: 'PART2_CONSENT', label: 'Part 2 Consent', required: true },
    { code: 'ROI_FACILITY', label: 'ROI (Facility)', required: true },
    { code: 'ROI_INSURANCE', label: 'ROI (Insurance)', required: true },
    { code: 'MH_783_APPLICATION', label: 'MH 783 Application', required: false, requiresCommitment: ['302', '303'] },
    { code: 'PHYSICIAN_EXAM_302', label: 'Physician Exam (302)', required: false, requiresCommitment: ['302'] },
    { code: 'COMMITMENT_ORDER_302', label: 'Commitment Order (302)', required: false, requiresCommitment: ['302'] },
    { code: 'COMMITMENT_ORDER_303', label: 'Commitment Order (303)', required: false, requiresCommitment: ['303'] },
    { code: 'INSURANCE_CARD', label: 'Insurance Card Copy', required: true },
    { code: 'PHOTO_ID', label: 'Photo ID Copy', required: true }
  ];

  const progressStages = [
    { key: 'assessment', label: 'Assessment', serverStatuses: ['DRAFT', 'ACTIVE'] },
    { key: 'outreach', label: 'Outreach', serverStatuses: ['SEARCHING', 'PENDING'] },
    { key: 'offers', label: 'Offers', serverStatuses: ['REVIEWING'] },
    { key: 'selected', label: 'Selected', serverStatuses: ['ACCEPTED'] },
    { key: 'scheduled', label: 'Scheduled', serverStatuses: ['SCHEDULED'] },
    { key: 'transferred', label: 'Transferred', serverStatuses: ['TRANSFERRED', 'CLOSED'] }
  ];

  function renderDashboard(patient) {
    if (!patient) {
      console.log('Dashboard: No patient selected');
      return;
    }

    // Patient Banner
    renderDashboardBanner(patient);

    // Progress Stages
    renderProgressStages(patient);

    // Facility Shortlist
    renderDashboardFacilities(patient);

    // Documents & Consents
    renderDashboardDocuments(patient);

    // PSP / Legal Tasks (302/303 only)
    renderDashboardPspTasks(patient);

    // Prior Auth / Benefits
    renderDashboardPriorAuth(patient);

    // Transport & Handoff
    renderDashboardTransport(patient);

    // Risks & Blockers
    renderDashboardRisks(patient);

    // Timeline
    renderDashboardTimeline(patient);

    // Team & Communication Log
    renderDashboardTeamComm(patient);
  }

  function renderDashboardBanner(patient) {
    const name = document.getElementById('dashboardPatientName');
    const mrn = document.getElementById('dashboardMrn');
    const dob = document.getElementById('dashboardDob');
    const asam = document.getElementById('dashboardAsam');
    const commitment = document.getElementById('dashboardCommitment');
    const sync = document.getElementById('dashboardSync');
    const lastAction = document.getElementById('dashboardLastAction');

    if (name) name.textContent = patient.identifiers?.name || patient.name || '—';
    if (mrn) mrn.textContent = patient.identifiers?.mrn || patient.id || '—';
    if (dob) dob.textContent = patient.dob || '—';
    if (asam) asam.textContent = patient.note?.asam || patient.asamLevel || '—';
    if (commitment) commitment.textContent = patient.note?.commitment || patient.commitmentStatus || 'None';
    ...
    if (lastAction) {
      ...
      if (activities.length > 0) {
        ...
        const timeAgo = getTimeAgo(latest.ts);
        lastAction.textContent = `${latest.action} (${timeAgo})`;
      } else {
  ...
  function renderProgressStages(patient) {
    const stagesContainer = document.getElementById('progressStages');
    if (!stagesContainer) return;

    // Determine current stage from patient status
    let currentStageKey = 'assessment';
    const searches = patient.searches || [];
    const hasSearches = searches.length > 0;
    const hasAccepted = searches.some(s => s.status === 'Accepted');
    const hasScheduled = patient.transport?.scheduled || false;
    const hasTransferred = patient.note?.transferStatus === 'transferred' || patient.status === 'TRANSFERRED';

    if (hasTransferred) {
      currentStageKey = 'transferred';
    } else if (hasScheduled) {
      currentStageKey = 'scheduled';
    } else if (hasAccepted) {
      currentStageKey = 'selected';
    } else if (hasSearches) {
      const hasPending = searches.some(s => s.status === 'PacketSent' || s.status === 'PendingReview');
      currentStageKey = hasPending ? 'offers' : 'outreach';
    }

    // Update stage highlighting
    const stageElements = stagesContainer.querySelectorAll('.stage');
    stageElements.forEach(el => {
      const testId = el.getAttribute('data-testid');
      const stageKey = testId?.replace('progress-stage-', '');
      el.classList.remove('active', 'complete');
      
      const stageIndex = progressStages.findIndex(s => s.key === stageKey);
      const currentIndex = progressStages.findIndex(s => s.key === currentStageKey);
    ...
    // Update metrics
    const timeOpen = document.getElementById('dashboardTimeOpen');
    const currentStage = document.getElementById('dashboardCurrentStage');
    const contacts = document.getElementById('dashboardContacts');
    const offers = document.getElementById('dashboardOffers');
    const declines = document.getElementById('dashboardDeclines');

    if (timeOpen) timeOpen.textContent = getTimeOpen(patient);
    if (currentStage) currentStage.textContent = progressStages.find(s => s.key === currentStageKey)?.label || '—';
    if (contacts) contacts.textContent = (patient.activities || []).filter(a => a.action === 'Call').length;
    if (offers) offers.textContent = searches.filter(s => s.status === 'PacketSent' || s.status === 'PendingReview').length;
    if (declines) declines.textContent = searches.filter(s => s.status === 'Denied' || s.status === 'NoBeds').length;
  }

  function renderDashboardFacilities(patient) {
    const container = document.getElementById('dashboardFacilityList');
    if (!container) return;

    const searches = patient.searches || [];
    const commitment = patient.note?.commitment || patient.commitmentStatus;

    // Apply commitment filtering rules
    let filteredSearches = searches;
    if (commitment === '302' || commitment === '303' || commitment === '302Required' || commitment === '303Required') {
      // Only show Psych Inpatient facilities that accept involuntary
      filteredSearches = searches.filter(search => {
        const facility = getFacilityById(search.facilityId);
        return facility?.locLevel === 'Psych Inpatient' && facility?.acceptsInvoluntary === true;
      });
    }

    if (filteredSearches.length === 0) {
      container.innerHTML = '<p class="empty-state">No facilities in active search</p>';
      return;
    }

    let html = '<table class="facility-table"><thead><tr><th>Facility</th><th>LOC</th><th>Status</th><th>Contact</th><th>Notes</th></tr></thead><tbody>';
    
    filteredSearches.forEach(search => {
      const facility = getFacilityById(search.facilityId);
      if (!facility) return;

      const statusInfo = statusConfig[search.status] || statusConfig.Searching;
      const rowClass = statusInfo.rowClass || '';

      html += `<tr class="${rowClass}">
        <td><strong>${facility.name}</strong></td>
        <td>${facility.locLevel || '—'}</td>
        <td><span class="${statusInfo.chipClass}">${statusInfo.icon} ${statusInfo.label}</span></td>
    ...
    // Add commitment filtering message if applicable
    if (commitment === '302' || commitment === '303' || commitment === '302Required' || commitment === '303Required') {
      const hiddenCount = searches.length - filteredSearches.length;
      if (hiddenCount > 0) {
        html += `<p class="commitment-filter-msg">⚠️ ${hiddenCount} ASAM facilities hidden (${commitment} requires Psych Inpatient + Involuntary)</p>`;
      }
    }
  ...
  function renderDashboardDocuments(patient) {
    const tbody = document.getElementById('dashboardDocsList');
    if (!tbody) return;
    ...
    let html = '';
    documentCodes.forEach(doc => {
      // Skip commitment-specific docs if not applicable
      if (doc.requiresCommitment && !doc.requiresCommitment.some(c => commitment?.includes(c))) {
        return;
      }

      const docData = documents[doc.code] || {};
      const status = docData.status || 'MISSING';
      const updated = docData.updated || '—';

      let statusClass = 'doc-missing';
      let statusLabel = 'Missing';
      if (status === 'RECEIVED' || status === 'SIGNED') {
        statusClass = 'doc-received';
        statusLabel = status === 'SIGNED' ? 'Signed' : 'Received';
      ...
      html += `<tr>
        <td><strong>${doc.code}</strong><br><small>${doc.label}</small></td>
        <td><span class="doc-status ${statusClass}">${statusLabel}</span></td>
        <td>${updated}</td>
        <td>
          <button class="ghost-btn" onclick="alert('Upload ${doc.code}')">Upload</button>
          <button class="ghost-btn" onclick="alert('View ${doc.code}')">View</button>
        </td>
      </tr>`;
    });
  ...
  function renderDashboardPspTasks(patient) {
    const container = document.getElementById('dashboardPspTasks');
    const listContainer = document.getElementById('dashboardPspList');
    if (!container || !listContainer) return;

    const commitment = patient.note?.commitment || patient.commitmentStatus;

    // Only show PSP section for 302/303
    if (!commitment || (!commitment.includes('302') && !commitment.includes('303'))) {
    ...
    const pspTasks = patient.pspTasks || [];
    if (pspTasks.length === 0) {
      listContainer.innerHTML = '<p class="empty-state">No PSP/Legal tasks assigned</p>';
      return;
    }

    let html = '<ul class="psp-task-list">';
    pspTasks.forEach(task => {
      const statusClass = task.completed ? 'task-complete' : 'task-pending';
      const icon = task.completed ? '✓' : '○';
      html += `<li class="${statusClass}">${icon} ${task.description} <small>(Due: ${task.due || 'TBD'})</small></li>`;
    });
    html += '</ul>';
  ...
  function renderDashboardPriorAuth(patient) {
    const plan = document.getElementById('dashboardPlan');
    const memberId = document.getElementById('dashboardMemberId');
    const eligibility = document.getElementById('dashboardEligibility');
    const authStatus = document.getElementById('dashboardAuthStatus');

    const pa = patient.priorAuth || {};
    const insurance = patient.insurance || {};

    if (plan) plan.textContent = pa.plan || insurance.plan || '—';
    if (memberId) memberId.textContent = insurance.memberNumber || '—';
    if (eligibility) eligibility.textContent = 'Active'; // Could be enhanced
    if (authStatus) {
      const status = pa.status || 'Not Started';
      authStatus.textContent = status;
      authStatus.className = '';
      if (status === 'APPROVED') authStatus.classList.add('auth-approved');
      else if (status === 'DENIED') authStatus.classList.add('auth-denied');
      else if (status.includes('PENDING') || status.includes('SUBMITTED')) authStatus.classList.add('auth-pending');
    }
  }

  function renderDashboardTransport(patient) {
    const mode = document.getElementById('dashboardTransportMode');
    const window = document.getElementById('dashboardTransportWindow');
    const receiving = document.getElementById('dashboardReceiving');
    const packet = document.getElementById('dashboardPacket');

    const transport = patient.transport || {};

    if (mode) mode.textContent = transport.mode || 'TBD';
    if (window) window.textContent = transport.window || '—';
    if (receiving) receiving.textContent = transport.receivingContact || '—';
    if (packet) packet.textContent = transport.packetStatus || 'Not sent';
  }

  function renderDashboardRisks(patient) {
    const container = document.getElementById('dashboardRisks');
    if (!container) return;
    ...
    const commitment = patient.note?.commitment || patient.commitmentStatus;
    if (commitment && (commitment.includes('302') || commitment.includes('303'))) {
      risks.push({ type: 'legal', text: `Active ${commitment} — PSP required` });
    }

    const pa = patient.priorAuth || {};
    if (pa.status === 'DENIED') {
      risks.push({ type: 'financial', text: 'PA DENIED — financial risk' });
    }

    const searches = patient.searches || [];
    const allDenied = searches.length > 0 && searches.every(s => s.status === 'Denied' || s.status === 'NoBeds');
    if (allDenied) {
      risks.push({ type: 'placement', text: 'All facilities denied/full — escalate' });
    }

    const documents = patient.documents || {};
    const missingDocs = documentCodes.filter(doc => {
      if (doc.requiresCommitment && !doc.requiresCommitment.some(c => commitment?.includes(c))) return false;
      const status = documents[doc.code]?.status || 'MISSING';
      return status === 'MISSING';
    });
    if (missingDocs.length > 0) {
      risks.push({ type: 'documents', text: `${missingDocs.length} documents missing` });
    }

    if (risks.length === 0) {
      container.innerHTML = '<p class="empty-state">No risks or blockers identified</p>';
      return;
    }

    let html = '';
    risks.forEach(risk => {
      let chipClass = 'risk-chip';
      if (risk.type === 'legal') chipClass += ' risk-legal';
      else if (risk.type === 'financial') chipClass += ' risk-financial';
      else if (risk.type === 'placement') chipClass += ' risk-placement';
      else if (risk.type === 'documents') chipClass += ' risk-documents';

      html += `<span class="${chipClass}">⚠ ${risk.text}</span>`;
    });

    container.innerHTML = html;
  }

  function renderDashboardTimeline(patient) {
    const container = document.getElementById('dashboardTimeline');
    if (!container) return;

    // Handle both array and object timeline formats
    ...
    const allEvents = [
      ...activities.map(a => ({ timestamp: a.ts, event: `${a.action}: ${a.notes || a.outcome}`, user: a.user || 'System' })),
      ...timelineSteps.map(t => t)
    ];

    if (allEvents.length === 0) {
      container.innerHTML = '<p class="empty-state">No timeline events recorded</p>';
      return;
    }
  ...
  function renderDashboardTeamComm(patient) {
    const container = document.getElementById('dashboardTeamComm');
    if (!container) return;

    const comm = patient.teamCommunications || [];
    if (comm.length === 0) {
      container.innerHTML = '<p class="empty-state">No team communications logged</p>';
      return;
    }

    let html = '<ul class="team-comm-list">';
    comm.forEach(msg => {
      html += `<li>
  ...
  function getFacilityById(id) {
    return facilityDirectoryData.find(f => f.id === id);
  }
  ...
  function getTimeOpen(patient) {
    if (!patient.note?.assessmentTs) return '—';
    const now = new Date();
    const created = new Date(patient.note.assessmentTs);
  ...
  async function handleSaveQuickUpdate() {
    const patient = getCurrentPatient();
    ...
    if (target === 'facility') {
      ...
      const { search, created } = await ensureFacilitySearch(patient, facilityId);
  ...
  async function saveEditPopover() {
    console.log('%c🔘 POPOVER SAVE BUTTON CLICKED', 'color: blue; font-weight: bold');
    
    if (!dom.editInfoPopover) return;
    const searchId = dom.editInfoPopover.dataset.searchId;
    const patientId = dom.editInfoPopover.dataset.patientId;
    
    // CRITICAL FIX: Use getCurrentPatient() instead of patientScenarios
    const patient = getCurrentPatient();
    if (!patient || patient.id !== patientId) {
      console.error('❌ Patient mismatch in popover');
      closeEditPopover();
      return;
    }
    ...
    const search = patient.searches.find((item) => item.id === searchId);
    if (!search) {
      console.error('❌ Search not found in patient');
      closeEditPopover();
      return;
    }
    
    console.log(`   Found search: ${search.facilityName}, current status: ${search.status}`);
    ...
    if (oldStatus !== statusValue) {
      console.log(`%c🎨 STATUS CHANGE (POPOVER): ${oldStatus} → ${statusValue} for ${search.facilityName}`, 'color: purple; font-weight: bold');
    }
    ...
    // CRITICAL FIX: Save to database so changes persist!
    await savePatientToDatabase(patient);
  ...
  async function persistDrawerChanges(publishToSsot = false) {
    console.log('%c💾 PERSIST DRAWER CHANGES CALLED', 'color: cyan; font-weight: bold');
    
    if (!dom.facilityDrawer) return;
    const searchId = dom.facilityDrawer.dataset.searchId;
    const patientId = dom.facilityDrawer.dataset.patientId;
    
    console.log(`   Search ID: ${searchId}, Patient ID: ${patientId}`);
    
    // Get patient from database or fallback
    const patient = getCurrentPatient();
    if (!patient || patient.id !== patientId) {
    ...
    if (dom.drawerStatus) {
      ...
      console.log(`%c🎨 STATUS CHANGE: ${oldStatus} → ${search.status} for ${search.facilityName}`, 'color: purple; font-weight: bold');
    }
    if (dom.drawerReason) {
  ...
  function handlePatientSwitch(event) {
    ...
    // Clear editing flag when switching patients - allow fresh assessment load
  ...
  function setupPrototypeActions() {
    ...
    if (dom.drawerSave) {
      dom.drawerSave.addEventListener('click', () => {
        console.log('%c🔘 DRAWER SAVE BUTTON CLICKED', 'color: blue; font-weight: bold');
        persistDrawerChanges(false);
      });
    }
    if (dom.drawerSaveChanges) {
      dom.drawerSaveChanges.addEventListener('click', () => {
        console.log('%c🔘 DRAWER SAVE CHANGES BUTTON CLICKED', 'color: blue; font-weight: bold');
        persistDrawerChanges(false);
      });
    }
    if (dom.drawerSavePublish) {
      dom.drawerSavePublish.addEventListener('click', () => {
        console.log('%c🔘 DRAWER SAVE & PUBLISH BUTTON CLICKED', 'color: blue; font-weight: bold');
        persistDrawerChanges(true);
      });
    }
    if (dom.editInfoPopover) {
...
Can I get a ticket/reference number and confirm the best callback/fax?"`;
  ...
  /**
   * Setup tracking for when user manually edits assessment fields
   * This prevents auto-reload from overwriting user changes
   */
  function setupAssessmentEditTracking() {
    ...
    assessmentFields.forEach((field) => {
      ...
      // Only track MANUAL changes (not programmatic ones)
      // Use 'input' event which only fires on user interaction
      field.addEventListener('input', () => {
        console.log('✏️ Assessment field changed by user:', field.id);
        userIsEditingAssessment = true;
      });
      
      // Also track when user makes a selection (for dropdowns)
