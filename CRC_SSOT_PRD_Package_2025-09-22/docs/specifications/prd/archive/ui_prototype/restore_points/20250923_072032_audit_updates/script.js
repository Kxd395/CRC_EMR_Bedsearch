(() => {
  ...
  function launchSearchPopover(options = {}) {
    if (!state.patient) return;
    ...
    openSearchPopover();
    if (options.bulk) {
      showToast(options.bulkMessage || 'Bulk placement search prototype — configure overrides, then Publish.');
    }
  }
  ...
  function setupEvents() {
    ...
    elements.addMultipleSearchBtn.addEventListener('click', () => {
      launchSearchPopover({ bulk: true, bulkMessage: 'Bulk placement search prototype — configure per facility manually.' });
    });
