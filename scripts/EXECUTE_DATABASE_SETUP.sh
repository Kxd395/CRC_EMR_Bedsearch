...
echo "=========================================="
echo "EMR Database Setup - Automated Execution"
echo "=========================================="
echo ""
echo "This will execute the database setup on your server."
...
echo "You can now test the connection from your Mac."
'
...
ssh -p 2222 kevindialmb@100.112.67.23 "echo '$SETUP_SCRIPT' > /tmp/setup_emr_db.sh && chmod +x /tmp/setup_emr_db.sh && /tmp/setup_emr_db.sh"
...
if [ $? -eq 0 ]; then
    ...
    echo "Testing database connection from Mac..."
    PGPASSWORD='emr_secure_2024' psql -h 100.112.67.23 -p 5432 -U emr_admin -d emr_crc_ssot -c 'SELECT 1;'
    
    if [ $? -eq 0 ]; then
        echo ""
        echo "🎉 SUCCESS! Database is connected and working!"
        echo ""
        echo "Next steps:"
        echo "1. Start your API server: npm run api"
        echo "2. Check logs for successful database connection"
        echo "3. Test the UI at http://localhost:5174"
        echo ""
    else
        echo ""
    ...
    echo ""
    echo "❌ Setup failed. Please check the SSH connection and try again."
    echo ""
fi