...
function loadStoredPanelVisibility() {
  ...
  async function loadPatientsFromDatabase() {
    ...
    try {
      console.log(`🔄 Attempting to load patients from database (attempt ${retryCount + 1}/${MAX_RETRIES})...`);
      ...
      loadedPatients = patients.map(dbPatient => ({
        ...
        activities: dbPatient.activities || [],
        audit: dbPatient.audit || [],
        timeline: dbPatient.timeline || { steps: [], activeIndex: 0 },
        board: dbPatient.board || {}
      }));
      ...
      console.error(`⚠️ Database load failed (attempt ${retryCount + 1}):`, error.message);
  ...
  async function savePatientToDatabase(patient) {
    try {
      ...
      if (index >= 0) {
        loadedPatients[index] = saved;
      } else {
        loadedPatients.push(saved);
      }
      ...
      // Schedule retry if database is available
      if (!databaseLoadFailed) {
        setTimeout(() => savePatientToDatabase(patient), 5000);
      }
