...
app.get('/api/patients/:id/assessments', async (req, res) => {
  try {
    ...
    try {
      const query = 'SELECT * FROM assessments WHERE patient_id = $1 ORDER BY created_at DESC';
    ...
    const assessments = await persistence.getAllFromFallback('assessments');
      
    const patientAssessments = assessments.filter(a => 
      a.patientId === id || a.patient_id === id
    );
    ...
    res.json({
      ...
      method: 'fallback',
      count: patientAssessments.length,
  ...
  try {
    ...
    if (result.success) {
      const patients = result.data.map((patient) => {
        ...
        return {
          id: patient.id,
          name: `${coalesce(patient.last_name, patient.lastName, 'Unknown')}, ${coalesce(patient.first_name, patient.firstName, 'Unknown')}`,
