(() => {
  ...
  const renderPatientList = (filteredData = null) => {
    ...
    const rows = data.map(patient => {
      ...
      tr.innerHTML = `
        <td class="col-patient"><div class="patient-name">${patient.patient || patient.name || ''}</div></td>
        <td class="col-placement">${patient.placement?.unit || patient.placement?.locTag || patient.placement?.text || 'Searching'}</td>
        <td class="col-mat">${patient.mat || ''}</td>
        <td class="col-status">${patient.status || ''}</td>
        <td class="col-transport">${patient.transport?.eta || patient.transport?.status || ''}</td>
        <td class="col-assigned">${patient.assigned?.name || patient.assigned || ''}</td>
        <td class="col-lastupdate">${patient.lastUpdate || ''}</td>
        <td class="col-reassess">${patient.reassess ? 'Y' : 'N'}</td>
      `;
      return tr;
    });
